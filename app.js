...
let fb = null;            // { db, storage, uid, ... }
let unsubDays = null, unsubCustom = null;
const pendingDays = new Set();
let pendingCustom = false;
const uploading = new Map(); // tempId -> {slot, name}
...
function day(k) {
  if (!state.days[k]) {
    state.days[k] = { date: k, items: [], photos: [], lunchNote: '', weight: null, fat: null };
  }
...
function recordedKeys() {
  return Object.keys(state.days).filter(k => {
    const d = state.days[k];
    return d && ((d.items && d.items.length) || d.weight != null || d.fat != null || (d.photos && d.photos.length));
  }).sort();
}
  ...
  for (const slot of SLOTS) {
    ...
    if (photos.length || pending.length) {
      ...
      photos.forEach(p => {
        ...
        const img = document.createElement('img'); img.src = p.url; img.alt = '食事の写真'; img.loading = 'lazy';
        img.addEventListener('click', () => openViewer(p.url));
        ...
        box.appendChild(img); box.appendChild(del); strip.appendChild(box);
      });
      pending.forEach(() => {
...
function renderHistory() {
  ...
  keys.forEach(k => {
    ...
    tr.innerHTML =
      `<td>${fmtMD(dt)}（${WD[dt.getDay()]}）${d.photos && d.photos.length ? ' 📷' : ''}</td>` +
...
async function compress(file, max, quality) {
  ...
  return new Promise(res => cv.toBlob(b => res({ blob: b, canvas: cv }), 'image/jpeg', quality));
}
async function handlePhoto(file, slot) {
  ...
  try {
    if (fb) {
      ...
      const path = `users/${fb.uid}/photos/${cur}/${tmp}.jpg`;
      const sref = fb.sRef(fb.storage, path);
      await fb.uploadBytes(sref, blob, { contentType: 'image/jpeg' });
      const url = await fb.getDownloadURL(sref);
      day(cur).photos.push({ id: tmp, slot, path, url });
      await saveDay(cur);
    } else {
      const { canvas } = await compress(file, 900, 0.6);
      const url = canvas.toDataURL('image/jpeg', 0.6);
      day(cur).photos.push({ id: tmp, slot, path: null, url });
      saveDay(cur);
    }
    toast('写真を追加しました');
  } catch (e) {
...
async function removePhoto(photoId) {
  ...
  render();
  if (ph && ph.path && fb) {
    try { await fb.deleteObject(fb.sRef(fb.storage, ph.path)); } catch (e) { /* 既に無い場合など */ }
  }
}
  ...
  authMod.onAuthStateChanged(auth, user => {
    if (user) {
      ...
      fb = {
        ...
        query: fsMod.query, orderBy: fsMod.orderBy, limit: fsMod.limit, onSnapshot: fsMod.onSnapshot,
        sRef: stMod.ref, uploadBytes: stMod.uploadBytes,
