...
const PORTIONS = [[2, '×2'], [1.5, '×1.5'], [1, '×1'], [0.75, '×3/4'], [2 / 3, '×2/3'], [0.5, '×1/2'], [1 / 3, '×1/3'], [0.25, '×1/4']];
let sheetQty = 1;
function qLabel(q) {
  const f = PORTIONS.find(x => Math.abs(x[0] - q) < 0.001);
  return f ? f[1] : '×' + r1(q);
}
function itemName(it) { return it.n + ((it.q && Math.abs(it.q - 1) > 0.001) ? ' ' + qLabel(it.q) : ''); }
...
function day(k) {
  if (!state.days[k]) {
    state.days[k] = { date: k, items: [], photos: [], lunchNote: '', weight: null, fat: null, muscle: null, training: { upper: false, lower: false } };
  }
  ...
  d.items.forEach(it => {
    if (it.bk === undefined) { it.bk = it.k; it.bp = it.p; it.q = 1; }
  });
  return d;
}
...
async function saveDay(k) {
  ...
  try {
    await fb.setDoc(fb.doc(fb.db, 'users', fb.uid, 'days', k), {
      ...
      lunchNote: d.lunchNote, weight: d.weight, fat: d.fat, muscle: d.muscle,
...
function recordedKeys() {
  return Object.keys(state.days).filter(k => {
    const d = state.days[k];
    return d && ((d.items && d.items.length) || d.weight != null || d.fat != null || d.muscle != null ||
...
// セットの中身は 'キー' か ['キー', 個数] のどちらでもよい
function specOf(x) { return Array.isArray(x) ? { id: x[0], q: x[1] } : { id: x, q: 1 }; }
function sumOf(list) {
  let k = 0, p = 0;
  list.forEach(x => { const s = specOf(x), f = FOODS[s.id]; if (f) { k += f.k * s.q; p += f.p * s.q; } });
  ...
  for (const slot of SLOTS) {
    ...
    if (!items.length) {
      ...
      items.forEach(it => {
        ...
        const n = document.createElement('div'); n.className = 'n'; n.textContent = itemName(it);
        n.style.cursor = 'pointer';
        n.title = 'タップで分量を変更';
        n.addEventListener('click', () => openQtySheet(it));
    ...
    SETS.filter(s => s.slot === slot.id).forEach(s => {
      act.appendChild(chip(s.name, sumOf(s.items), 'key', () => addMany(s.items, slot.id, s.name)));  // セットは×1で追加（各項目は後から調整できる）
    });
    if (slot.id === 'dinner') {
...
function ensureSaveUI() {
  ...
  const anchor = $('wIn').closest('.bodyrow');
...
function renderDaySummary() {
  ...
  const dt = parseKey(cur);
  const head = document.createElement('div');
  head.className = 'st';
  head.textContent = `${dt.getMonth() + 1}月${dt.getDate()}日（${WD[dt.getDay()]}）の記録`;
  box.appendChild(head);

  let any = false;
  for (const slot of SLOTS) {
    const items = (d && d.items ? d.items : []).filter(it => it.slot === slot.id);
    const note = (slot.id === 'lunch' && d && d.lunchNote) ? d.lunchNote : '';
    if (!items.length && !note) continue;
    any = true;
    const st = slotTotals(d, slot.id);

    const h = document.createElement('div');
    h.className = 'row';
    h.style.borderBottom = '1px solid var(--line)';
    h.innerHTML = `<div class="n" style="font-weight:700">${slot.name}</div>` +
      `<div class="k">${Math.round(st.k)}kcal</div><div class="p">P${r1(st.p)}</div>`;
    h.style.padding = '8px 0';
    box.appendChild(h);

    if (note) {
      const nn = document.createElement('div');
      nn.style.cssText = 'font-size:13px;color:var(--muted);padding:6px 0 4px;white-space:pre-wrap';
      nn.textContent = note;
      box.appendChild(nn);
    }
    items.forEach(it => {
      ...
      r.className = 'row';
      r.style.cssText = 'padding:6px 0;border-bottom:none';
      r.innerHTML = `<div class="n">${itemName(it)}</div>` +
        `<div class="k">${Math.round(it.k)}kcal</div><div class="p">P${r1(it.p)}</div>`;
      box.appendChild(r);
    });
  }
  ...
  if (!any) {
    const e = document.createElement('div');
    e.style.cssText = 'font-size:13px;color:var(--muted);padding:6px 0';
    e.textContent = 'この日の記録はまだありません。';
    box.appendChild(e);
    return;
  }
  ...
  tot.className = 'row';
  tot.style.cssText = 'padding:10px 0 0;border-top:1px solid var(--line);border-bottom:none;margin-top:6px';
  tot.innerHTML = '<div class="n" style="font-weight:700">合計</div>' +
    `<div class="k" style="font-weight:700">${Math.round(t.k)}kcal</div>` +
    `<div class="p" style="font-weight:700">P${r1(t.p)}</div>`;
  box.appendChild(tot);

  const extra = [];
  if (d && d.weight != null) extra.push(`体重 ${d.weight}kg`);
  if (d && d.fat != null) extra.push(`体脂肪率 ${d.fat}%`);
  if (d && d.muscle != null) extra.push(`筋肉量 ${d.muscle}kg`);
  const tl = trainLabel(d);
  if (tl) extra.push('トレーニング ' + (tl === '上下' ? '上半身・下半身' : tl === '上' ? '上半身' : '下半身'));
  if (extra.length) {
...
function renderHistory() {
  ...
  if (!keys.length) {
    tb.innerHTML = '<tr><td colspan="6" style="text-align:center;color:var(--muted)">記録がありません</td></tr>';
    return;
  }
  keys.forEach(k => {
...
function renderChart() {
  ...
  const ms = pts.map(k => state.days[k].muscle == null ? null : +state.days[k].muscle);
  ...
  if (seg.length > 1) s += `<polyline points="${seg.join(' ')}" fill="none" stroke="var(--kcal)" stroke-width="1.5" stroke-dasharray="3 3"/>`;
  const mv = ms.filter(v => v != null);
  if (mv.length > 1) {
    let mseg = [];
    const mmin0 = Math.min(...mv), mmax0 = Math.max(...mv);
    const mmin = (mmax0 - mmin0 < 1) ? mmin0 - 0.5 : mmin0, mmax = (mmax0 - mmin0 < 1) ? mmax0 + 0.5 : mmax0;
    const ym = v => PT + (mmax - v) / (mmax - mmin) * (H - PT - PB);
    ms.forEach((v, i) => {
      if (v == null) {
        if (mseg.length > 1) s += `<polyline points="${mseg.join(' ')}" fill="none" stroke="var(--prot)" stroke-width="1.5"/>`;
        mseg = [];
      } else mseg.push(x(i) + ',' + ym(v));
    });
    if (mseg.length > 1) s += `<polyline points="${mseg.join(' ')}" fill="none" stroke="var(--prot)" stroke-width="1.5"/>`;
  }
...
/* ========== 操作 ========== */
function addMany(list, slot, label, q) {
  const mul = q || 1;
  const d = day(cur);
  list.forEach(x => {
    const sp = specOf(x), f = FOODS[sp.id];
    if (!f) return;
    const qq = sp.q * mul;
    d.items.push({ id: uid(), slot, n: f.n, bk: f.k, bp: f.p, q: qq, k: f.k * qq, p: f.p * qq });
  });
  saveDay(cur); render(); toast(label + (mul !== 1 ? ' ' + qLabel(mul) : '') + ' を追加');
}
function addOne(name, k, p, slot, q) {
  const mul = q || 1;
  day(cur).items.push({ id: uid(), slot, n: name, bk: k, bp: p, q: mul, k: k * mul, p: p * mul });
  saveDay(cur); render(); toast(name + (mul !== 1 ? ' ' + qLabel(mul) : '') + ' を追加');
}
// 追加済みの項目の分量を変える（食べ残して翌日に回すとき用）
function setItemQty(itemId, q) {
  const it = day(cur).items.find(x => x.id === itemId);
  if (!it) return;
  it.q = q; it.k = it.bk * q; it.p = it.bp * q;
  saveDay(cur); render();
}
// 分量を選ぶ小さいシート
function openQtySheet(it) {
  const wrap = document.createElement('div');
  wrap.className = 'sheet';
  wrap.innerHTML = '<div class="sheetbg"></div><div class="sheetbody">' +
    '<div class="sheethead"><strong></strong><button class="linkbtn">閉じる</button></div>' +
    '<div class="chips" style="padding:12px"></div>' +
    '<p class="note" style="padding:0 12px 14px">元の分量：' +
    Math.round(it.bk) + 'kcal ／ P' + r1(it.bp) + '</p></div>';
  wrap.querySelector('strong').textContent = it.n + ' の分量';
  const close = () => wrap.remove();
  wrap.querySelector('.sheetbg').addEventListener('click', close);
  wrap.querySelector('.linkbtn').addEventListener('click', close);
  const box = wrap.querySelector('.chips');
  PORTIONS.forEach(([v, label]) => {
    const b = chip(label, Math.round(it.bk * v) + 'kcal/P' + r1(it.bp * v),
      Math.abs((it.q || 1) - v) < 0.001 ? 'key' : '',
      () => { setItemQty(it.id, v); close(); });
    box.appendChild(b);
  });
...
/* ========== 追加シート ========== */
let sheetSlot = null;
function ensureQtyRow() {
  if (document.getElementById('qtyRow')) return;
  const row = document.createElement('div');
  row.id = 'qtyRow';
  row.className = 'chips';
  row.style.cssText = 'padding:0 12px 10px';
  $('sheetSearch').insertAdjacentElement('afterend', row);
}
function renderQtyRow() {
  ensureQtyRow();
  const row = document.getElementById('qtyRow');
  row.innerHTML = '';
  const lab = document.createElement('span');
  lab.style.cssText = 'font-size:12px;color:var(--muted);align-self:center;margin-right:2px';
  lab.textContent = '分量';
  row.appendChild(lab);
  PORTIONS.forEach(([v, label]) => {
    row.appendChild(chip(label, '', Math.abs(sheetQty - v) < 0.001 ? 'key' : '', () => {
      sheetQty = v; renderQtyRow(); renderSheetList($('sheetSearch').value);
    }));
  });
}
...
function renderSheetList(q) {
  ...
  const push = (name, k, p, onDel) => {
    ...
    const b = chip(name, Math.round(k * sheetQty) + 'kcal/P' + r1(p * sheetQty), '', () => {
      addOne(name, k, p, sheetSlot.id, sheetQty); closeSheet();
    });
  ...
  unsubDays = fb.onSnapshot(fb.query(daysCol, fb.orderBy('date', 'desc'), fb.limit(500)), snap => {
    snap.forEach(docSnap => {
      ...
      state.days[docSnap.id] = {
        ...
        muscle: v.muscle === undefined ? null : v.muscle,
...
$('mIn').addEventListener('change', e => bodyChange('muscle', e.target));
